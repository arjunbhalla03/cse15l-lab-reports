...
2. When cd is called on its own, it will backtrack to the directory that the current directory is in. For us, that results in us leaving the lecture1 directory and going back to the main desktop.
3. There isn't an error as it does go back and send us to the previous as it is supposed to.

*with directory as its argument*
1. The working directory was the main desktop in edstem that is being used for the lab.
...
1. The working directory was the main desktop in edstem that was being used in the lab.
2. When cat was called without any arguments, it had no idea what to read and output. So as we can see, the command outputs nothing to the command line and returns blank.
3. There is an error here as cat does need some input to read data from and without it nothing will happen and will cause the terminal to not respond.
...
3. There is no error as the program is aware it is a directory and cannot read the directory the same way it would read a normal file.
